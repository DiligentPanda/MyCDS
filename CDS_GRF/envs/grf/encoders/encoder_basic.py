...
class FeatureEncoder:
    ...
    def global_state_space(self):
        return Box(low=-1000,high=1000,shape=[4*self.num_players+14])
    ...
    def get_global_state(self,state):
        ...
        left_status = np.concatenate([
            obs['left_team'].flatten()*2, # 2*self.num_players
            obs['left_team_direction'].flatten()*100 # 2*self.num_players
            ]
        )
        right_status = np.concatenate([
            obs['right_team'].flatten()*2, # 2*self.num_players
            obs['right_team_direction'].flatten()*100 # 2*self.num_players
            ]
        )
        ball_status = np.concatenate([
            obs['ball'],                # 3
            obs['ball_direction']*20    # 3
            ]
        )
        game_mode = np.zeros(7, dtype=np.float32) # 7
        game_mode[obs["game_mode"]] = 1
        score_ratio = obs["score"][0] - obs["score"][1] # 1
        score_ratio /= 5.0
        score_ratio = min(score_ratio, 1.0)
        score_ratio = max(-1.0, score_ratio)
        match_state = np.concatenate( # 8
            (
                np.array([score_ratio]),
                game_mode,
            )
        )

        # 4*num_players+14
        global_state = np.concatenate([left_status, right_status, ball_status, match_state])
        return global_state
